...
def format():
    ...
    if not all(element.isalpha() for element in user_input):
        result_label.config(text="Enter a valid word!!")
        return

    input_consonants = [letter for letter in user_input if letter in consonants]

    for index in range(math.floor(len(input_consonants) / 2)) :
        if input_consonants[index] != input_consonants[-index-1]:
...
Опис ПЗ: 
ПЗ перевіряє слово, чи є воно приголосним палідромом
'''
...
task_label = tk.Label(root, text="Enter a word:", font=("Arial", 12), bg=dark_bg, fg=dark_fg)
